...
struct QuestionPlayerView: View {
    ...
    /// Scroll target for the verdict, so answering doesn't leave the explanation
    /// below the fold behind the pinned Next button.
    ...
    private func verdictView(_ question: Question, proxy: ScrollViewProxy) -> some View {
        if let picked = session.pickedIndex {
            ...
            // A long prompt plus four options fills the screen, so the verdict
            // and the "Why" card land below the fold — and with the Next button
            // pinned over the bottom edge there isn't even a scroll cue.
            //
            // Driven from `onAppear` rather than `onChange(of: session.isAnswered)`:
            // at the moment the flag flips this view doesn't exist yet, so the
            // scroll target can't be resolved and the scroll falls short.
            .onAppear {
                withAnimation(
